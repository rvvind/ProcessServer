...
public class Redirect
{
	...
	public Response redirectSolutionChange(@MatrixParam("paramName") String name, @MatrixParam("paramVal") String value)
	{
		try
		{
			...
			PreparedStatement sqlQuery = con.prepareStatement("select distinct row_id from dbo.R3_S_PROD_DEFECT (NOLOCK) where " + paramName + " = ?");// l.lookUp("clientSourceQuery",
			// "db.properties"));
			...
			@MatrixParam("destination") String destination, @MatrixParam("viewName") String viewName)
	{
		try
		{
			...
			PreparedStatement sqlQuery = con.prepareStatement("select distinct row_id as id from dbo.R3_S_ORG_EXT (NOLOCK) where " + paramName + " = ?");// l.lookUp("clientSourceQuery",
			// "db.properties"));
			sqlQuery.setString(1, value);
	...
	private Response processParamsAndExecDB(String source, String name, String value) throws Exception
	{
			...
			if (paramName != null)
			{
				if (!paramName.isEmpty() || !paramName.equalsIgnoreCase(""))
				{
					...
					PreparedStatement sqlQuery1 = con
							.prepareStatement("select distinct INSTANCEID from RemedyIntegration.dbo.VW_HPD_HELP_DESK (NOLOCK) where " + paramName + " = ?");
		...
		else if (source.equalsIgnoreCase("odscrm"))
		{
			...
			if (paramName != null)
			{
				...
				{
					Connection con = odsCon.connect(source);
					PreparedStatement sqlQuery = con.prepareStatement("select distinct row_id as id from dbo.R3_S_SRV_REQ (NOLOCK) where " + paramName
							+ " = ?");
					sqlQuery.setString(1, value);
